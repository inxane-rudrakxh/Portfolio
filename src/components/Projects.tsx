  ...
  return (
    <section className="relative z-20 bg-[#0a0a0a] min-h-screen py-20 md:py-32 px-4 md:px-12 overflow-hidden" id="projects">
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <motion.div 
            layout 
            className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 auto-rows-[minmax(220px,auto)] md:auto-rows-[minmax(280px,auto)]"
        >
                    ...
                    >
                        ...
                        <div className="absolute inset-0 p-5 md:p-8 flex flex-col justify-between z-10">
                            <div className="flex justify-between items-start">
                                 <span className="inline-block px-2.5 py-1 rounded-full bg-black/40 border border-white/10 text-[10px] md:text-xs font-mono text-blue-300 backdrop-blur-md max-w-[75%] truncate">
                                    {project.category}
                                 </span>
                            ...
                            <div>
                                <h3 className="text-lg md:text-2xl font-bold text-white mb-2 leading-tight group-hover:translate-x-1 transition-transform drop-shadow-lg">{project.title}</h3>
                                <p className="text-gray-200 text-xs md:text-sm line-clamp-2 md:line-clamp-3 leading-relaxed opacity-90 group-hover:opacity-100 transition-opacity drop-shadow-md">
                                    {project.description}
                                </p>
        ...
        <AnimatePresence>
            {selectedId && selectedProject && (
                <>
                    ...
                    />
                    <div className="fixed inset-0 flex items-end md:items-center justify-center z-70 pointer-events-auto p-0 md:p-8">
                        <motion.div
                           layoutId={selectedId}
                           className="bg-[#121212] w-full max-w-4xl max-h-[95vh] md:max-h-[90vh] overflow-y-auto rounded-t-4xl md:rounded-4xl border border-white/10 shadow-2xl relative scrollbar-hide"
                         >
                           <button 
                                onClick={() => setSelectedId(null)}
                                className="absolute top-4 right-4 md:top-6 md:right-6 z-20 p-2 bg-black/50 hover:bg-black/80 rounded-full text-white/70 hover:text-white transition-colors border border-white/10"
                            >
                           ...
                           <div className="flex flex-col md:flex-row h-full">
                                {/* Visual Side */}
                                <div className={`w-full md:w-2/5 h-48 md:min-h-[300px] relative overflow-hidden flex flex-col justify-end p-6 md:p-8`}>
                                    <img 
                                   ...
                                   <motion.span 
                                     ...
                                     className="relative z-10 inline-block px-3 py-1 rounded-full bg-black/40 text-xs font-mono text-white mb-3 w-fit border border-white/10 backdrop-blur-md"
                                   >
                                ...
                                {/* Content Side */}
                                <div className="w-full md:w-3/5 p-6 md:p-12 bg-[#121212]">
                                    <motion.div
                                        initial={{ opacity: 0 }}
                                        animate={{ opacity: 1 }}
                                        transition={{ delay: 0.4 }}
                                    >
                                        <h4 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-3">About the project</h4>
                                        <p className="text-gray-300 leading-relaxed mb-6 text-base md:text-lg">
                                            {selectedProject.longDescription}
                                        </p>

                                        <div className="mb-8">
                                            <h4 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-3">Core Technologies</h4>
                                            <div className="flex flex-wrap gap-2">
                                                {selectedProject.techStack.map((tech, i) => (
                                                    <motion.span 
                                                        ...
                                                        className="px-3 py-1.5 rounded-xl bg-white/5 hover:bg-white/10 text-xs md:text-sm text-gray-200 border border-white/5 transition-colors cursor-default"
                                                    >
                                                        {tech}
                                                    </motion.span>
                                                ))}
                                        ...
                                        <div className="flex gap-3 pt-4 border-t border-white/10">
                                            <a 
                                                ...
                                                className="flex-1 py-3 md:py-4 rounded-xl bg-white text-black font-bold text-center hover:bg-gray-200 transition-colors flex items-center justify-center gap-2 text-sm md:text-base"
                                            >
                                            ...
                                            <a 
                                                ...
                                                target="_blank" 
                                                rel="noopener noreferrer" 
                                                className="flex-1 py-3 md:py-4 rounded-xl bg-white/5 text-white font-bold text-center hover:bg-white/10 transition-colors border border-white/10 flex items-center justify-center gap-2 text-sm md:text-base"
                                            >
                                                Live Demo
